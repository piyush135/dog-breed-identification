...
for i, img_id in enumerate(df_labels['img_file']):
  ...
  img1 = cv2.imread(str(train_file)+img_id,cv2.IMREAD_COLOR)
  if img1 is not None and img1.shape[0] > 0 and img1.shape[1] > 0:
   img = cv2.resize(img1,((im_size,im_size)))
